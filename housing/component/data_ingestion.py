...
class DataIngestion:
    ...
    def download_housing_data(self)->str:
        try:
            ...
            tgz_file_name=os.path.basename(dataset_download_url)

            tgz_file_path=os.path.join(tgz_download_dir,tgz_file_name)

            logging.info(f"Downloading dataset from :[{dataset_download_url}] into dir:[{tgz_file_path}]")
            urllib.request.urlretrieve(dataset_download_url,tgz_file_path)
            urllib.request.urlretrieve
            logging.info(f"File:[{tgz_file_name}] has been downloaded successfully.")
    ...
    def extract_tgz_file(self,tgz_file_path:str)->None:
        try:
            ...
            logging.info(f"Extracting file from: [{tgz_file_path}] into dir:[{raw_data_dir}]")
            with tarfile.open(tgz_file_path,'r') as tgz_file:
                 ...
                 safe_extract(tgz_file, path=raw_data_dir)
